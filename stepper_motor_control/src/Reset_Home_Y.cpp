...
void call_sdo_write_and_init(const std::shared_ptr<rclcpp::Node>& node) {
    // Client for /third_shaft_joint/sdo_write service
...
// Set behavior of limit switch activation to no reaction 
    ...
    auto sdo_write_client_limit = node->create_client<canopen_interfaces::srv::COWrite>("/third_shaft_joint/sdo_write");
...
// Call init service
    ...
    // Client for /third_shaft_joint/init service
    auto init_client = node->create_client<std_srvs::srv::Trigger>("/third_shaft_joint/init");

    // Wait for the /init service to be available
    if (!init_client->wait_for_service(std::chrono::seconds(5))) {
        RCLCPP_ERROR(node->get_logger(), "/third_shaft_joint/init service not available.");
        return;
    }
    ...
    if (rclcpp::spin_until_future_complete(node, init_result) != rclcpp::FutureReturnCode::SUCCESS) {
        RCLCPP_ERROR(node->get_logger(), "Failed to call /third_shaft_joint/init service.");
        return;
    }
    RCLCPP_INFO(node->get_logger(), "Successfully called /third_shaft_joint/init service.");
}

int main(int argc, char *argv[]) {
    rclcpp::init(argc, argv);
    auto node = rclcpp::Node::make_shared("Reset_Home_Y");
