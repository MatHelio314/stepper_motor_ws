...
void joint_state_callback_1(const sensor_msgs::msg::JointState::SharedPtr msg) {
    if (!msg->position.empty()) {
        ...
        RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Motor 4 current position: %.2f", current_position_);
    }
}

int main(int argc, char *argv[]) {
    rclcpp::init(argc, argv);
    auto node = rclcpp::Node::make_shared("position_tick_motor_Z_node");

    // Initialize clients 
    auto init_client_ = node->create_client<std_srvs::srv::Trigger>("/fourth_shaft_joint/init");
    auto mode_client_ = node->create_client<std_srvs::srv::Trigger>("/fourth_shaft_joint/position_mode");
    auto halt_client_ = node->create_client<std_srvs::srv::Trigger>("/fourth_shaft_joint/halt");
    auto target_client_ = node->create_client<canopen_interfaces::srv::COTargetDouble>("/fourth_shaft_joint/target");
